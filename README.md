# MusicCenter

Веб-пульт для Raspberry Pi: показывает библиотеку с домашнего Airsonic-сервера
(артисты, альбомы, поиск), а звук воспроизводит **сам Raspberry Pi** через
`mpv`, подключённый аудиовыходом к музыкальному центру. Управлять можно с
любого устройства в домашней сети (телефон, ноутбук) — открыв в браузере
`http://<IP-адрес-Pi>:5000`.

## Требования

- Raspberry Pi (или любой Linux-хост), подключённый аудиовыходом
  (jack/HDMI/Bluetooth) к музыкальному центру
- Python 3.10+
- `mpv`, установленный в системе: `sudo apt install mpv`
- Работающий Airsonic / Navidrome / другой Subsonic-совместимый сервер в той
  же сети

## Установка и запуск

```bash
git clone <репозиторий> MusicCenter
cd MusicCenter

cp .env.example .env
# отредактируйте .env - впишите AIRSONIC_URL/USERNAME/PASSWORD

set -a; source .env; set +a
./run.sh
```

`run.sh` сам создаст виртуальное окружение, поставит зависимости из
`requirements.txt` и запустит сервер через gunicorn на `0.0.0.0:5000`.

Для разработки можно запускать напрямую:

```bash
pip install -r requirements.txt
python app.py
```

## Настройка

Все параметры — в [`config.py`](config.py), большинство можно переопределить
переменными окружения (см. [`.env.example`](.env.example)):

| Переменная | Назначение | По умолчанию |
|---|---|---|
| `AIRSONIC_URL` | адрес Airsonic-сервера | `http://airsonic.local:4040` |
| `AIRSONIC_USERNAME` / `AIRSONIC_PASSWORD` | логин/пароль | пусто — обязательно задать |
| `PLAYER_DEFAULT_VOLUME` | громкость mpv при старте (0-100) | `70` |
| `PLAYER_FADE_SECONDS` | длительность фейд-перехода между треками, сек | `2.5` |
| `PORT` | порт веб-интерфейса | `5000` |
| `DEBUG` | режим отладки Flask | `false` |

Версия Subsonic REST API, которую понимает сервер, может отличаться — если
после запуска библиотека не загружается, проверьте `AIRSONIC_API_VERSION` в
`config.py` (для Airsonic-Advanced обычно подходит `1.15.0`).

## Автозапуск (systemd)

Чтобы сервер поднимался сам при включении Pi и перезапускался при падении:

```bash
sudo cp deploy/musiccenter.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now musiccenter
```

Юнит ожидает, что проект лежит в `/home/admin/MusicCenter`, а `.env` с
настройками — рядом (`/home/admin/MusicCenter/.env`). Если путь или
пользователь другие — поправьте `deploy/musiccenter.service` перед копированием.

Проверить статус и логи:

```bash
sudo systemctl status musiccenter
sudo journalctl -u musiccenter -f
```

## Telegram-бот

Отдельный процесс (`bot/main.py`), управляет плеером через тот же REST API,
что и сайт (`/player/*`, `/search/api`), по `http://127.0.0.1:5000` — не
трогает Airsonic/mpv напрямую и не требует, чтобы сайт был открыт где-либо.

1. Создайте бота через **@BotFather** (`/newbot`), получите токен.
2. Узнайте свой Telegram ID через **@userinfobot** (и остальных, кому нужен
   доступ).
3. В `.env`:
   ```
   TELEGRAM_BOT_TOKEN=<токен от BotFather>
   TELEGRAM_ALLOWED_IDS=123456789,987654321
   ```
   Без ID в списке бот отвечает "Доступ запрещён" — это не публичный бот.
4. Запуск вручную (для проверки):
   ```bash
   set -a; source .env; set +a
   python -m bot.main
   ```
5. Автозапуск через systemd — так же, как для сайта:
   ```bash
   sudo cp deploy/musiccenter-bot.service /etc/systemd/system/
   sudo systemctl daemon-reload
   sudo systemctl enable --now musiccenter-bot
   ```

Команды: `/now` — что играет + кнопки play/pause/prev/next/громкость,
`/search <запрос>` — найти альбом/трек и запустить кнопкой из списка.

**Уведомления о смене трека** — бот сам ничего не шлёт: у него нет фонового
процесса, привязанного к плееру, только ответы на команды. Пуш о смене
трека шлёт веб-процесс напрямую в Telegram Bot API (тем же `TELEGRAM_BOT_TOKEN`/
`TELEGRAM_ALLOWED_IDS` из `.env`), минуя `bot/main.py`, поэтому боту не
обязательно быть запущенным. Включается тумблером на странице
[`/settings`](#настройки-прямо-из-веб-интерфейса), по умолчанию выключено.

## Настройки прямо из веб-интерфейса

Страница `/settings` (ссылка в сайдбаре) — часть настроек, которые раньше
требовали править `.env` и перезапускать сервис руками:

- **Громкость и эквалайзер по умолчанию** — применяются сразу, без
  перезапуска (плеер уже запущен, команды идут через тот же IPC, что и
  обычное управление).
- **Аудиовыход mpv** (`PLAYER_AUDIO_OUTPUT`) — сохраняется, но применяется
  только при следующем перезапуске mpv-процесса (`sudo systemctl restart
  musiccenter`), т.к. `--ao` — параметр запуска, на лету не переключается.
- **Тумблер Telegram-уведомлений** — см. "Telegram-бот" выше.

Настройки хранятся в `data/settings.json` (не в `.env` и не в git) — переживают
`git pull`, ничего вручную поддерживать не нужно.

**PIN-доступ.** На той же странице можно задать PIN — после этого
веб-интерфейс из локальной сети требует вход на `/login`. Пока PIN не
задан, доступ как раньше — без пароля. Не распространяется на:
`/kiosk` (экран на самом Pi, не рассчитан на доступ с других устройств) и
запросы с `127.0.0.1` (так Telegram-бот дёргает `/player/*` — у него своя
защита через `TELEGRAM_ALLOWED_IDS`, PIN он не проходит и не должен).

## Экран на самом Pi (киоск-режим)

Есть отдельная компактная страница `http://<IP-Pi>:5000/kiosk` — большая
обложка, название/исполнитель, часы, play/pause/prev/next. Без сайдбара и
навигации, вёрстка резиновая (`vh`/`vw`/`clamp`), рассчитана на маленький
экран (типично 3.5", ~480×320), но не привязана к конкретному разрешению.

**Выбор экрана.** Проще всего — 3.5" экран, где GPIO только для питания/тача,
а видео идёт по отдельному mini-HDMI кабелю: он определяется как обычный
монитор, без возни с драйверами фреймбуфера под конкретную модель. Если
берёте "чистый" SPI/GPIO-экран (Waveshare и т.п., видео через GPIO) — там
нужен драйвер под конкретную модель, напишите, какая именно, когда выберете.

**Настройка автозапуска в киоск-режим** (Raspberry Pi OS Lite, без рабочего
стола — меньше накладных расходов для устройства, которое всегда показывает
одно и то же):

```bash
sudo apt install --no-install-recommends -y \
    xserver-xorg x11-xserver-utils xinit openbox chromium-browser unclutter

sudo raspi-config
# System Options → Boot / Auto Login → Console Autologin
```

В конец `~/.bash_profile` (создайте, если нет):
```bash
if [ -z "$DISPLAY" ] && [ "$(tty)" = "/dev/tty1" ]; then
    startx
fi
```

Создайте `~/.xinitrc`:
```bash
xset -dpms
xset s off
xset s noblank
unclutter -idle 0.5 -root &
openbox-session &
chromium-browser \
    --kiosk \
    --noerrdialogs \
    --disable-infobars \
    --disable-session-crashed-bubble \
    --check-for-update-interval=31536000 \
    http://localhost:5000/kiosk
```

`sudo reboot` — Pi должен загрузиться прямо в это окно, без рабочего стола и
без возможности из него выйти (что и нужно для приборной панели).

## Плавные переходы и эквалайзер

Работает сразу, без настройки. mpv запускается с `--gapless-audio=yes`
(без тишины между треками), плюс короткий fade-out в конце каждого трека и
fade-in в начале следующего (`PLAYER_FADE_SECONDS`, по умолчанию 2.5с) —
звучит почти как кроссфейд, но без риска, что два трека "поедут" друг на
друга, как бывает с настоящим overlapping-кроссфейдом через IPC-управляемый
mpv. EQ-пресеты (обычный/бас/вокал/высокие/V-образный) — в полноэкранном
плеере, применяются мгновенно через тот же `af`-фильтр.

## Живая спектрограмма (опционально, экспериментально)

По умолчанию спектрограмма в плеере — декоративная анимация (звук играет
через `mpv` на самом Pi, а не в браузере, так что обычный Web Audio API тут
не работает). Но раз mpv всё равно на этой же машине — можно дать
приложению читать реальный аудиопоток через ALSA-loopback и вместо анимации
рисовать настоящие уровни громкости. Это меняет системный аудио-конфиг
Pi, поэтому **полностью опционально и по умолчанию выключено** — без этой
настройки ничего не меняется, звук идёт как раньше.

**Предупреждение по опыту:** маршрутизация звука через `type multi` +
loopback (шаги ниже) на практике оказалась нестабильной — трек мог играть
первые 1-2 секунды и обрываться, даже с `plughw`-обёрткой слейвов для
конвертации частоты. Причина не докопана до конца (похоже на проблему с
`type multi` под настоящей нагрузкой, а не только на синтетическом
`speaker-test`). Если звук важнее декоративной анимации — не включайте это,
`PLAYER_AUDIO_OUTPUT=alsa` без loopback полностью надёжен. Настраивайте
только если готовы потратить время на отладку и не боитесь временно
остаться без звука.

1. Загрузить loopback-модуль и сделать это постоянным:
   ```bash
   sudo modprobe snd-aloop
   echo snd-aloop | sudo tee -a /etc/modules
   ```
2. Создать `/etc/asound.conf` с ALSA `multi`-устройством, которое играет
   одновременно на настоящую карту и на loopback (посмотрите номер вашей
   карты через `aplay -l`, ниже `hw:0,0` — обычно тот же, что было в
   `PLAYER_AUDIO_OUTPUT`):
   ```
   pcm.multi {
       type multi
       slaves.a.pcm "hw:0,0"
       slaves.a.channels 2
       slaves.b.pcm "hw:Loopback,0,0"
       slaves.b.channels 2
       bindings.0.slave a
       bindings.0.channel 0
       bindings.1.slave a
       bindings.1.channel 1
       bindings.2.slave b
       bindings.2.channel 0
       bindings.3.slave b
       bindings.3.channel 1
   }
   pcm.multi_route {
       type route
       slave.pcm multi
       slave.channels 4
       ttable.0.0 1
       ttable.1.1 1
   }
   ```
3. В `.env`:
   ```
   PLAYER_AUDIO_OUTPUT=alsa/multi_route
   SPECTRUM_CAPTURE_DEVICE=hw:Loopback,1,0
   ```
4. Установить опциональные зависимости в то же виртуальное окружение (они
   намеренно **не** в `requirements.txt`, чтобы не ломать установку для
   всех остальных):
   ```bash
   sudo apt install libasound2-dev
   source venv/bin/activate
   pip install numpy pyalsaaudio
   ```
5. `sudo systemctl restart musiccenter` — если всё поднялось,
   `sudo journalctl -u musiccenter -n 20` не покажет
   `SpectrumAnalyzer: capture device unavailable`, а спектрограмма в
   полноэкранном плеере/на киоск-экране начнёт реагировать на реальный
   звук вместо равномерной анимации. Если что-то не так — приложение само
   тихо откатится на декоративную анимацию, ничего не сломается.

Чувствительность можно подстроить без правки кода через `SPECTRUM_GAIN` в
`.env` (меньше значение — спектрограмма "живее" при тихом звуке).

## Last.fm-скробблинг

Полностью фоновый — работает без интерфейса, просто отмечает "сейчас
играет" и скробблит трек в ваш профиль Last.fm (после 50% трека или 4
минут, что раньше — как в официальных клиентах).

1. Создайте приложение на <https://www.last.fm/api/account/create> —
   получите `API_KEY` и `API_SECRET`.
2. Разовая авторизация — получить постоянный `LASTFM_SESSION_KEY`:
   ```bash
   python scripts/lastfm_auth.py
   ```
   Скрипт спросит `API_KEY`/`API_SECRET`, откроет ссылку на last.fm для
   подтверждения (или просто выведет её — откройте с телефона/другого
   браузера), и после `Enter` напечатает готовую строку для `.env`. Можно
   запускать с любой машины с Python и интернетом, не обязательно с Pi.
3. В `.env`:
   ```
   LASTFM_API_KEY=...
   LASTFM_API_SECRET=...
   LASTFM_SESSION_KEY=...
   ```
4. `sudo systemctl restart musiccenter`.

## Возможности

- Библиотека: артисты, альбомы, поиск с живыми подсказками, сортировка
  альбомов (новые/алфавит/исполнитель/год/случайно) и исполнителей
  (алфавит/по числу альбомов)
- Плеер: очередь, shuffle, повтор (off/all/one), громкость, перемотка
- Гэплесс + фейд-переход между треками, 5 EQ-пресетов (см. "Плавные
  переходы и эквалайзер")
- Спектрограмма в плеере — по умолчанию декоративная, можно включить
  живую от реального аудиопотока (см. "Живая спектрограмма")
- Last.fm-скробблинг (см. "Last.fm-скробблинг")
- **Radio-режим** — когда очередь заканчивается, подтягивает похожие треки
  (`getSimilarSongs2`), а если сервер не отдал похожих — просто продолжает
  случайными треками из библиотеки
- **Таймер сна** — ставится в полноэкранном плеере (иконка 🌙), выключает
  паузу через 15/30/45/60 минут; переживает закрытие браузера, так как
  таймер живёт на сервере, а не на клиенте
- **Избранное** — ★ на альбомах/исполнителях/треках
- **Плейлисты** — создание, добавление/удаление треков, воспроизведение
- Полноэкранный "Сейчас играет" — тап по обложке в мини-плеере
- Устанавливается как приложение на телефон (PWA) — "Добавить на экран"
  в браузере (Android/Chrome и iOS/Safari — своя PNG-иконка)
- Управление с экрана блокировки телефона (play/pause/prev/next/перемотка)
  через Media Session API, пока вкладка с сайтом открыта в фоне
- Отдельный компактный экран `/kiosk` для маленького дисплея на самом Pi
- Настройки по умолчанию (громкость/EQ/аудиовыход) и опциональный
  PIN-доступ прямо со страницы `/settings` (см. "Настройки прямо из
  веб-интерфейса"), без правки `.env`

## Если звука нет

- Проверьте, что `mpv` установлен: `mpv --version`. Если бинарник не найден,
  плеер на сайте покажет баннер "mpv не найден на этом устройстве" — сайт при
  этом продолжает работать (можно смотреть библиотеку и искать), но
  воспроизведение недоступно.
- Тот же баннер появляется и когда `mpv` установлен, но не может стартовать —
  в логе (`sudo journalctl -u musiccenter -n 40`) это выглядит как
  `mpv did not create its IPC socket in time`. Частая причина: **сервис
  запущен через systemd, а не в интерактивном логине**, поэтому у mpv нет
  доступа к пользовательской сессии PipeWire/PulseAudio, через которую он
  обычно выводит звук, и он падает при попытке открыть аудио-устройство.
  Решение уже встроено — mpv по умолчанию выводит звук через ALSA напрямую
  (`PLAYER_AUDIO_OUTPUT=alsa` в `.env`), в обход PipeWire/Pulse. Если у вас
  несколько аудиоустройств (HDMI/джек/USB) и звук идёт не туда, посмотрите
  доступные карты через `aplay -l` и укажите конкретную:
  `PLAYER_AUDIO_OUTPUT=alsa/hw:1,0`.
- Убедитесь, что аудиовыход Pi по умолчанию — тот самый, что подключён к
  музыкальному центру (`raspi-config` → System Options → Audio, либо
  `amixer` / `pactl` в зависимости от дистрибутива).
- В `run.sh` намеренно используется один воркер gunicorn (`--workers 1`) —
  очередь воспроизведения и процесс `mpv` держатся в памяти одного процесса,
  несколько воркеров запустили бы несколько независимых плееров.

## Структура проекта

- `app.py` — точка входа Flask, регистрация blueprint'ов
- `api/airsonic.py` — клиент Subsonic REST API
- `api/lastfm.py` — клиент Last.fm API (подпись запросов, scrobble)
- `services/` — бизнес-логика: `airsonic_service.py` (нормализация данных),
  `artwork.py` (кэш обложек), `queue.py` (очередь воспроизведения),
  `lastfm_service.py` (скробблинг, no-op без настройки), `telegram_notify.py`
  (пуш о смене трека, no-op без настройки/выключенного тумблера),
  `settings_service.py` (чтение/запись `data/settings.json`)
- `core/player.py` — управление локальным `mpv` через JSON IPC, фейд/EQ
- `core/spectrum.py` — живая спектрограмма из ALSA-loopback (опционально)
- `routes/` — HTTP-роуты (страницы + `/player/*`, `/favorites/*`,
  `/playlists/*` REST API), `settings.py` (`/settings`), `auth.py`
  (`/login`, `/logout`)
- `templates/`, `static/` — интерфейс
- `bot/` — Telegram-бот, отдельный процесс поверх REST API сайта
- `scripts/lastfm_auth.py` — разовый CLI для получения Last.fm session key
- `deploy/musiccenter.service`, `deploy/musiccenter-bot.service` —
  systemd-юниты для автозапуска
- `.env.example` — шаблон переменных окружения (реальный `.env` в
  `.gitignore`, туда секреты не попадут)
- `data/settings.json` — настройки со страницы `/settings` (тоже в
  `.gitignore` — runtime-состояние, не код)
